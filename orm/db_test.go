...
func OpenTestConnection() (db *micros.DB, err error) {

	fmt.Println("testing mysql...")
	dbhost := os.Getenv("micros_DBADDRESS")
	dbhost = fmt.Sprintf("tcp(%v)", dbhost)
	db, err = orm.Open("mysql", fmt.Sprintf("micros:micros@%v/micros?charset=utf8&parseTime=True", dbhost))
...
func TestOpenExistingDB(t *testing.T) {
	...
	db, err := micros.Open(dialect, DB.DB())
	if err != nil {
		t.Errorf("Should have wrapped the existing DB connection")
	}

	var user User
	if db.Where("name = ?", "jnfeinstein").First(&user).Error == micros.ErrRecordNotFound {
...
func BenchmarkRawSql(b *testing.B) {
	DB, _ := sql.Open("postgres", "user=micros DB.ame=micros sslmode=disable")
